...
class NyayikNSchat extends StatefulWidget {
  final data;
  NyayikNSchat(this.data);
  @override
  _NyayikNSchatState createState() => _NyayikNSchatState();
}

class _NyayikNSchatState extends State<NyayikNSchat> {
  dynamic datalist;
  ...
  Future<Getmessage> messageApi() async {
    Getmessage? message = await getMsgFromUserNSApi();
  ...
  storemessage(a) {
    ...
    storechatNSApi(data).then((value) => {});
      ...
      child: Scaffold(
        ...
        appBar: AppBar(
          ...
          title: Row(
            ...
            children: <Widget>[
              MyCircleAvatar(
                imgUrl: '$user_image${widget.data['data']['image']}',
              ),
              SizedBox(width: 15),
              Column(
                    ...
                    itemBuilder: (ctx, i) {
                      if ('${data1[i].sender_id}' != '${userdata['data']['id']}') {
                        var v = {
                          ...
                          'userimage': '${widget.data['data']['user_img']}'
                        };
