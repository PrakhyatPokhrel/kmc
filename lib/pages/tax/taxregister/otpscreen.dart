...
class Otp extends StatefulWidget {
  final dynamic entry_id;

  const Otp({this.entry_id});
  @override
...
class _OtpState extends State<Otp> with SingleTickerProviderStateMixin {
  ...
  validateSubmitOTP() {
    ...
    if (otp.length == 6) {
      taxRegisterSubmit(data).then((value) => {
            EasyLoading.dismiss(),
            if (value["status"] == true)
              {
                ...
                EasyLoading.showSuccess("tax register message".tr,
                    duration: Duration(
                      seconds: 5,
                    )),
              }
